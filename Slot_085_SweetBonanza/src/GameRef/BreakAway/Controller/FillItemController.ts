class FillItemController {
	private _adjustmentFillPos: number = 0;
	private _itemMap: ItemMap = null;
	...
	public async FillItems(slotItems: Array<SlotItem>, _lenght: number = 20) {
		...
		for (let i = 0; i < this._itemMap.MapSize[1]; i++) {
			...
			for (let j = 0; j < this._itemMap.ItemRow[i]; j++) {
				...
				let needBonuce = this.UseFillSlotItem[idxCount].y != mapPos.y;
				if (needBonuce) {
					idxs.push(idxCount);
				}
				idxCount++;
		...
		for(let i = 0;i<this.updateTimes;++i)
		{
			this.UpdateFillId.push(EventManager.Instance.RegisterEventListener(UpdateEvent, this, this.UpdateFill));	
		}
	...
	public EndFill() {
		for(let i = 0;i<this.UpdateFillId.length;++i)
		{
			let id = this.UpdateFillId.shift();
	...
	public SetRunMoveList(slotItems: Array<SlotItem>, idxsArray: Array<Array<number>>, _lenght) {
		...
		let AddTime = 0;
		for (let i = idxsArray.length - 1; i >= 0; i--) {
